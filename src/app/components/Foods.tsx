          ...
          <div className="grid grid-cols-4 gap-2">
            {category.foods && category.foods.length > 0 ? (
              category.foods.map((food) => (
                <div
                  key={food._id}
                  className="border p-2 border-gray-400 rounded-lg bg-white h-full "
                >
                  {food.image && (
                    <img
                      src={food.image as string}
                  ...
                  <div className="flex flex-col justify-between items-start mt-2 relative">
                    <div className="flex justify-between w-full">
                      {/* Food Name */}
                      <p
                        className="text-red-500 font-bold text-start truncate w-[75%] hover:overflow-visible hover:h-full hover:whitespace-normal hover:bg-white "
                        title={food.foodName}
                      >
                        {food.foodName}
                      </p>
                      ...
                      <button
                        className="absolute top-[-60px] right-4 p-3 bg-white cursor-pointer rounded-full shadow"
                        onClick={() => handleEdit(food)}
                      >
                        <PenIcon className="text-red-500 w-5 h-5" />
                      </button>
                    ...
                    <p className="font-bold ">
                      Ingredients:
                      <span className="font-normal pl-2 text-wrap h-20 hover:h-full">
                        {food.ingredients}
                      </span>
                    </p>
                  </div>
